...
function buildOnSettled(done) {
  ...
  function onSettled(error, result) {
    ...
    var settledErrors = _.filter(result, { state: 'error' });
    var settledResults = _.filter(result, { state: 'success' });

    var errors = null;
    if (settledErrors.length) {
      errors = _.map(settledErrors, 'value');
    }

    var results = null;
    if (settledResults.length) {
      results = _.map(settledResults, 'value');
    }
